...
describe("pre-battle command planning", () => {
  ...
  it("rejects planned commands for units outside the issuing faction", () => {
    ...
    const initial = createBattleState([friendly, enemy]);
    ...
    expect(engaged).toBe(initial);
    expect(engaged.units.find((unit) => unit.id === enemy.id)?.order).toEqual({ type: "idle" });
  });
});
