...
describe("battle spatial index", () => {
  ...
  it("excludes dead units and supports caller-defined threat filters", () => {
    const origin = createBattleUnit({
      id: "origin",
      faction: "verdant",
      role: "mage",
    ...
    expect(index.enemiesWithin(origin, 3, {
      include: (unit) => unit.role === "knight",
    }).map((unit) => unit.id)).toEqual(["knight"]);
  });
});
